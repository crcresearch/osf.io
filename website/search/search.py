...
@requires_search
def raw_get(id, index=None, doc_type=None): 
    index=index or settings.ELASTIC_INDEX   
    return search_engine.raw_get(id, index=index, doc_type=doc_type) 

@requires_search
def raw_search(query, index=None, doc_type=None):
    index = index or settings.ELASTIC_INDEX
    return search_engine.raw_search(query, index=index, doc_type=doc_type) 

@requires_search
def raw_index(body, id=None, index=None, doc_type=None):
    index = index or settings.ELASTIC_INDEX
    return search_engine.raw_index(body, index=index, doc_type=doc_type, id=id)

@requires_search
def raw_delete(id, index=None, doc_type=None):
    index = index or settings.ELASTIC_INDEX
    return search_engine.raw_delete(id, index=index, doc_type=doc_type)

@requires_search
def raw_update(body, index=None, doc_type=None, id=None):
    index = index or settings.ELASTIC_INDEX
    return search_engine.raw_update(body, id, index=index, doc_type=doc_type)
